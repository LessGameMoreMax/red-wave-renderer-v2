...
public:
    ...
    Gbuffer(const unsigned int width, const unsigned int height){
        ...
        // 创建并绑定 MetalRoughAo Buffer
        glGenTextures(1, &gMetalRoughAo);
        glBindTexture(GL_TEXTURE_2D, gMetalRoughAo);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGBA, GL_FLOAT, NULL);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, gMetalRoughAo, 0);

        // 告知OpenGL使用此Framebuffer时应输出的attachment
